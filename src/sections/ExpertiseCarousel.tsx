  ...
  return (
    ...
    >
      <div className={viewMode === "carousel" ? "w-full" : "mx-auto max-w-7xl px-6"}>
        ...
        >
          {viewMode === "grid" && <GridView isInView={isInView} />}
          {viewMode === "carousel" && <CarouselView />}
          {viewMode === "cards" && <CardsView isInView={isInView} />}
        </motion.div>
      </div>
    </section>
  );
...
function GridView({ isInView }: { isInView: boolean }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
...
function CardsView({ isInView }: { isInView: boolean }) {
  return (
