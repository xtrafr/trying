 ...
 * @author xtra
 * 
 * glass effect on secondary, glowing shadow on primary,
 * spring animations make it feel responsive
 */
...
export default function Button({
  ...
  const Component = href ? motion.a : motion.button; // render as link or button
  ...
  return (
    <Component
      ...
      className={cn(
        ...
        variant === "primary" &&
          "border hover:brightness-110 active:scale-[0.98]", // primary gets brighter
        variant === "secondary" &&
          "bg-transparent text-text border hover:active:scale-[0.98]", // secondary is transparent
        className
      )}
      style={{
      ...
      whileTap={{ scale: 0.98, y: 0 }} // satisfying press feedback
      transition={{ type: "spring", stiffness: 400, damping: 25 }}
    >
      <span className="relative z-10">{children}</span>
      {/* gradient overlay for secondary buttons */}
      {variant === "secondary" && (
        <motion.div
