...
/**
 * Hero Section, the first thing people see when they land
 * @author xtra
 * 
 * Got that smooth gradient background with a subtle grid,
 * the availability badge pulses (looks sick),
 * scroll indicator is pure liquid glass vibes
 */
...
export default function Hero() {
  ...
  // Apple's easing, makes everything feel buttery
  const smoothEase = [0.22, 1, 0.36, 1] as const;
  
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden px-4 sm:px-6 pt-24 sm:pt-20 pb-12 sm:pb-16">
      {/* subtle gradient that adds some depth */}
      <div className="absolute inset-0">
        ...
        />
        {/* noise texture makes it feel premium af */}
      ...
      <div className="relative z-10 mx-auto max-w-5xl text-center">
        {/* availability badge with that ping animation */}
        <motion.div
        ...
        >
          <span className="relative flex h-2.5 w-2.5">
            {/* the ping effect that actually looks cool */}
            <span 
        ...
        {/* main title - clamp() keeps it responsive */}
        <motion.h1
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ 
        ...
        <motion.p
          ...
          className="mx-auto mb-10 max-w-3xl text-[clamp(1.125rem,2vw,1.375rem)] text-text-secondary leading-[1.6] px-2 sm:px-4"
        >
          {t.hero.description}
        </motion.p>
        ...
        {/* scroll indicator - liquid glass style */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ 
        ...
        >
          <motion.div
            animate={{ y: [0, 12, 0] }} // bounces smoothly
          ...
          >
            ...
            {/* glass mouse with frosted effect */}
            <div 
            ...
            >
              ...
              {/* little dot that scrolls inside */}
              <motion.div 
                ...
                animate={{ y: [0, 32, 0] }} // scroll animation loop
                transition={{ duration: 3, repeat: Infinity, ease: smoothEase }}
              />
            </div>
          </motion.div>
