...
// In a production environment, you'd use a database like Vercel KV, MongoDB, etc.
...
export default async function handler(req, res) {
  // Set CORS headers - allow requests from any origin (adjust in production)
  ...
  if (req.method === 'POST') {
    try {
      ...
      if (type === 'pageview') {
        ...
        // Also track as connection if IP info available
        if (data.ip) {
          const connection = {
            id: Date.now() + Math.random(),
            ip: data.ip,
